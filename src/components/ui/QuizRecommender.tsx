...
type CompanySize = 'smb' | 'mid' | 'ent' | '';

const isCompanySize = (value: string): value is CompanySize =>
  value === '' || value === 'smb' || value === 'mid' || value === 'ent';
    ...
    <form onSubmit={onSubmit} className="rounded-2xl border border-white/10 bg-white/5 p-6 dark:bg-black/20">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="text-sm">
          <label htmlFor="quiz-role" className="block">
            What are you exploring?
          </label>
          <select
            id="quiz-role"
            className="mt-1 w-full rounded-md bg-transparent p-2 outline-none ring-1 ring-white/10"
            value={role}
            onChange={(event) => setRole(event.target.value as Ans)}
          >
        ...
        <div className="text-sm">
          <label htmlFor="quiz-size" className="block">
            Company size
          </label>
          <select
            id="quiz-size"
